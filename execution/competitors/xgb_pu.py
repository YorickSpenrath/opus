...
class PUBoost:
    ...
    def fit(self, x_train, y_train, x_val=None, y_val=None):
        if self.obj == 'ce':
            ...
            xgboost = xgb.train(params=self.params, dtrain=dtrain, num_boost_round=self.n_estimators,
            ...
            def upu_train(raw_scores, y_true):
                ...
                # Gradient
                grad = -y * (1.0 / label_freq_) * (1 - scores) + y * (1.0 - 1.0 / label_freq_) * scores + (
                        1 - y) * scores
            ...
            def upu_val(raw_scores, y_true):
                ...
                loss[pos] = -1.0 * ((1.0 / label_freq_) * np.log(scores[pos]) + (1.0 - 1.0 / label_freq_) * np.log(
                    1.0 - scores[pos]))
